...
3 (above the 'tr'), 4 (surrounded by parens) and 5 (cue notehead in parens). The default setting is 1.
@Accidental
...
local function draw_Trill(t)
	...
	if acc ~= '' then
		...
		local y1 = scale*(yo-sf)
		nwcdraw.setFontSize(nwcdraw.getFontSize()*sf)
		local ax = x1+trLen
		if accStyle == 3 then
			ax = x1+(trLen-spLen-nwcdraw.calcTextSize(acc))*0.5
		elseif accStyle == 4 then
			acc = '(_' .. acc .. '_)' 
		elseif accStyle == 5 then
			nwcdraw.setFontClass('StaffCueSymbols')
			ax = startNote:xyRight()+0.5
			y1 = startNote:notePos(1)+3.5
			local y2 = tonumber(string.match(startNote:notePitchPos(1),'-?%d+'))+1
			--print (y2)
			if math.abs(y2) > 5 then
				local y4 = y2<0 and 0 or 1
				--local y3 = y1-2.5 + (y2+1) % 2
				--local y3 = y1-2.5 + (y2+y4) % 2 + y4
				local y3 = y1-3.5 + (y2+1) % 2
				nwcdraw.line(ax+1, y3, ax+2.1, y3)
				acc = '(_' .. acc .. '__k__)'
			else
				acc = '(_' .. acc .. '_k_)'
			end
		end
		if accStyleVars[3] then accLen = nwcdraw.calcTextSize(acc) end
		nwcdraw.moveTo(ax, y1)
		nwcdraw.text(acc)
		len = len - accLen
	end
