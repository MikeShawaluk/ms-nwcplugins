...
values is 8 (slow) to 128 (very fast), with a default setting of 32 (32nd notes).
@WhichFirst
...
if nwcut then
    ...
    local prevUserItem = nil
    local spanningStart = nil

    local function applyTrill(o)
        if o:IsFake() then
            if o.UserType == userObjTypeName and not prevUserItem then
                prevUserItem = o.Opts
            end
            return
        end
        ...
        elseif o:ContainsNotes() then
            ...
            if action == 'Default' or action == 'Add' then
                local newUserItem = nwcItem.new('|User|'..userObjTypeName)
                newUserItem:Provide('Scale', '100')
                newUserItem:Provide('Span', '0')
                if prevUserItem then
                    -- Copy parameter values from a prior trill (except Span)
                    for k,v in pairs(prevUserItem) do
                        newUserItem.Opts[k] = v
                    end
                    newUserItem.Opts['Span'] = 0
                end
                opts.Muted = ''
                action = 'Add'
                -- Compute the trill marker Y position
                local topPos = -1000
                local tieOut = false
                for notepos in o:AllNotePositions() do
                    local pos = notepos.Position
                    if pos > topPos then
                        topPos = pos
                        -- Check if the topmost note has a tie out
                        tieOut = string.find(o:__tostring(), '^', 1, true)
                    end
                end
                if topPos < 3 then -- Trill marker always above the staff
                    topPos = 3
                end
                newUserItem.Opts.Pos = topPos + 7*newUserItem.Opts['Scale']/100
                local noTrillMarker = false
                if tieOut then
                    if spanningStart then
                        spanningStart:Provide('Span', '2')
                        spanningStart.Opts['Span'] = spanningStart.Opts['Span'] + 1
		        noTrillMarker = true
                    else
                        newUserItem.Opts['Span'] = 2
                        spanningStart = newUserItem
                    end
                else
		            noTrillMarker = spanningStart
                    spanningStart = nil
                end
                if noTrillMarker then
                     return { o }
                end
                return { newUserItem, o }
    ...
    if action == 'Default' then
        nwcut.msgbox(('No notes or chords found for %s'):format(userObjTypeName))
    else
        score:save()
    end
    return
