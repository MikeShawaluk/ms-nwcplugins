...
-- The embedded "Toggle" user tool will clone parameter values from a prior arpeggio.
-- Those preferring the original tool behavior can use "Toggle (old)".
    local function applyArpeggio(o)
        if o:IsFake() then
            if o.UserType == userObjTypeName and not prev then
                prev = o.Opts
            end
            return
        end
        ...
        elseif o:ContainsNotes() then
            ...
            for notepos in o:AllNotePositions() do
                if playContext:FindTieIndex(notepos) then
                  tiedIn = true
                end
            end
            playContext:put(o)

            if noteobjTypes[o.ObjType] then -- single notes are skipped
